	...
	public function index()
	{
		if($this->input->post()){
			...
			if (count($data)>=1){
				...
				$u_mstats 		= $data[0]['u_mstats'];
				$u_estats 		= $data[0]['u_estats'];
				...
				$this->session->set_userdata('tel_no',$tel_no);
				...
				$this->session->set_userdata('u_mstats',$u_mstats);
				$this->session->set_userdata('u_estats',$u_estats);
	...
	public function signup()
	{
		if($this->input->post()){
			...
			if($pass_id == $cnf_pass_id){
				...
				$data_check=$this->db->query("SELECT * FROM `users_tbl` WHERE `email_id` = '$email_id' OR `tel_no` = '$tel_no'");
